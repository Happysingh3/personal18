...
from odoo.tools import format_time, format_duration
...
class HrAttendance(models.Model):
    ...
    break_hours = fields.Float(string='Break Hours', compute='_compute_break_stats', store=False)
    break_count = fields.Integer(string='Breaks', compute='_compute_break_stats', store=False)

    # ✅ naye columns jo list me dikhayenge
    last_break_in = fields.Datetime(string="Break In", compute='_compute_break_columns', store=False)
    last_break_out = fields.Datetime(string="Break Out", compute='_compute_break_columns', store=False)
    # (optional) saare breaks ek line me
    breaks_summary = fields.Char(string="Breaks (all)", compute='_compute_break_columns', store=False)
    @api.depends('break_ids.check_in', 'break_ids.check_out')
    def _compute_break_stats(self):
        for att in self:
        # count
         att.break_count = len(att.break_ids)
        # sum duration if your break model stores it; else compute from times
        total = 0.0
        for b in att.break_ids:
            if getattr(b, "duration", False):
                total += b.duration
            elif b.check_in and b.check_out:
                total += (b.check_out - b.check_in).total_seconds() / 3600.0
        att.break_hours = round(total, 2)
    @api.depends('break_ids.check_in', 'break_ids.check_out')
    def _compute_last_break_times(self):
        for att in self:
            if att.break_ids:
            # pick the latest break by check_in (handles None safely)
             recs = att.break_ids.filtered(lambda r: r.check_in).sorted(key=lambda r: r.check_in)
             last = recs[-1] if recs else att.break_ids[-1]
             att.last_break_in = last.check_in
             att.last_break_out = last.check_out
        else:
            att.last_break_in = False
            att.last_break_out = False
    
    @api.depends('break_ids.check_in', 'break_ids.check_out')
    def _compute_break_columns(self):
        for att in self:
            brks = att.break_ids.sorted('check_in')
            if brks:
                att.last_break_in = brks[-1].check_in
                att.last_break_out = brks[-1].check_out
                # optional summary like "12:55 → 13:10 | 16:00 → 16:20"
                parts = []
                for b in brks:
                    s = b.check_in and fields.Datetime.to_string(b.check_in)[11:16] or ''
                    e = b.check_out and fields.Datetime.to_string(b.check_out)[11:16] or ''
                    parts.append(f"{s} → {e}")
                att.breaks_summary = " | ".join(parts)
            else:
                att.last_break_in = False
                att.last_break_out = False
                att.breaks_summary = ""
    ...
    def _compute_display_name(self):
        tz = request.httprequest.cookies.get('tz') if request else None
        for attendance in self:
            emp = attendance.employee_id.name or _("Employee")
            if not attendance.check_out:
                # Open shift
                attendance.display_name = _(
                    "%(emp)s — at %(check_in)s",
                    ...
                    check_in=format_time(
                        self.env, attendance.check_in, time_format=None, tz=tz, lang_code=self.env.lang
                    ),
                )
            else:
                # Closed shift
                attendance.display_name = _(
                    "%(emp)s — %(worked_hours)s (%(check_in)s-%(check_out)s)",
                    emp=emp,
                    worked_hours=format_duration(attendance.worked_hours),
                    check_in=format_time(
                        self.env, attendance.check_in, time_format=None, tz=tz, lang_code=self.env.lang
                    ...
                    check_out=format_time(
                        self.env, attendance.check_out, time_format=None, tz=tz, lang_code=self.env.lang
                    ),
                )
    ...
    @api.depends('check_in', 'check_out', 'break_ids.check_in', 'break_ids.check_out')
    def _compute_worked_hours(self):
        """Worked Hours = (check_out - check_in) - SUM(overlapping breaks)"""
        ...
        for att in self:
            ...
            end = att.check_out or now
            total = (end - att.check_in).total_seconds() / 3600.0

            # minus only the part of each break that overlaps this attendance
            minus_breaks = 0.0
            for br in att.break_ids:
                if not br.check_in:
                    continue
                br_start = br.check_in
                br_end = br.check_out or end
                # overlap window
                start = max(att.check_in, br_start)
                stop = min(end, br_end)
                if start < stop:
                    minus_breaks += (stop - start).total_seconds() / 3600.0

            att.worked_hours = max(total - minus_breaks, 0.0)
    ...
    def write(self, vals):
        ...
        if self.env.context.get('_skip_approve_update'):
            result = super(HrAttendance, self).write(vals)
            if any(field in vals for field in ['employee_id', 'check_in', 'check_out']):
                for emp, dates in self._get_attendances_dates().items():
                    attendances_dates[emp] |= dates
        ...
        if 'overtime_status' in vals:
            to_set = self.filtered(lambda r: r.overtime_status == 'approved' and not r.approved_by)
            if to_set:
                to_set.with_context(_skip_approve_update=True).write({
