# -*- coding: utf-8 -*-
# Part of Odoo. See LICENSE file for full copyright and licensing details.

import pytz
from dateutil.relativedelta import relativedelta

from odoo import models, fields, api, exceptions, _
from odoo.exceptions import UserError


class HrEmployee(models.Model):
    _inherit = "hr.employee"

    # ------------------------------------------------------------
    # FIELDS
    # ------------------------------------------------------------
    attendance_manager_id = fields.Many2one(
        'res.users',
        store=True,
        readonly=False,
        domain="[('share', '=', False), ('company_ids', 'in', company_id)]",
        groups="hr_attendance.group_hr_attendance_officer",
        help="The user set in Attendance will access the attendance of the employee "
             "through the dedicated app and will be able to edit them.",
    )
    attendance_ids = fields.One2many(
        'hr.attendance', 'employee_id',
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )
    last_attendance_id = fields.Many2one(
        'hr.attendance', compute='_compute_last_attendance_id', store=True,
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )
    last_check_in = fields.Datetime(
        related='last_attendance_id.check_in', store=True,
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user", tracking=False,
    )
    last_check_out = fields.Datetime(
        related='last_attendance_id.check_out', store=True,
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user", tracking=False,
    )
    attendance_state = fields.Selection(
        string="Attendance Status", compute='_compute_attendance_state',
        selection=[('checked_out', "Checked out"), ('checked_in', "Checked in")],
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )
    hours_last_month = fields.Float(
        compute='_compute_hours_last_month',
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )
    hours_today = fields.Float(
        compute='_compute_hours_today',
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )
    hours_previously_today = fields.Float(
        compute='_compute_hours_today',
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )
    last_attendance_worked_hours = fields.Float(
        compute='_compute_hours_today',
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )
    hours_last_month_display = fields.Char(
        compute='_compute_hours_last_month', groups="hr.group_hr_user",
    )
    overtime_ids = fields.One2many(
        'hr.attendance.overtime', 'employee_id',
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )
    total_overtime = fields.Float(
        compute='_compute_total_overtime', compute_sudo=True,
        groups="hr_attendance.group_hr_attendance_officer,hr.group_hr_user",
    )

    # ------------------------------------------------------------
    # CRUD
    # ------------------------------------------------------------
    @api.model_create_multi
    def create(self, vals_list):
        officer_group = self.env.ref('hr_attendance.group_hr_attendance_officer', raise_if_not_found=False)
        group_updates = []
        for vals in vals_list:
            if officer_group and vals.get('attendance_manager_id'):
                group_updates.append((4, vals['attendance_manager_id']))
        if group_updates:
            officer_group.sudo().write({'users': group_updates})
        return super().create(vals_list)

    def write(self, values):
        old_officers = self.env['res.users']
        if 'attendance_manager_id' in values:
            old_officers = self.attendance_manager_id
            if values['attendance_manager_id']:
                officer = self.env['res.users'].browse(values['attendance_manager_id'])
                officers_group = self.env.ref('hr_attendance.group_hr_attendance_officer', raise_if_not_found=False)
                if officers_group and not officer.has_group('hr_attendance.group_hr_attendance_officer'):
                    officer.sudo().write({'groups_id': [(4, officers_group.id)]})

        res = super(HrEmployee, self).write(values)
        old_officers.sudo()._clean_attendance_officers()
        return res

    # ------------------------------------------------------------
    # COMPUTES
    # ------------------------------------------------------------
    @api.depends('overtime_ids.duration', 'attendance_ids', 'attendance_ids.overtime_status')
    def _compute_total_overtime(self):
        mapped_validated_overtimes = dict(self.env['hr.attendance']._read_group(
            domain=[('overtime_status', '=', 'approved')],
            groupby=['employee_id'],
            aggregates=['validated_overtime_hours:sum'],
        ))
        mapped_overtime_adjustments = dict(self.env['hr.attendance.overtime']._read_group(
            domain=[('adjustment', '=', True)],
            groupby=['employee_id'],
            aggregates=['duration:sum'],
        ))
        for employee in self:
            employee.total_overtime = (
                mapped_validated_overtimes.get(employee, 0)
                + mapped_overtime_adjustments.get(employee, 0)
            )

    def _compute_hours_last_month(self):
        now = fields.Datetime.now()
        now_utc = pytz.utc.localize(now)
        for employee in self:
            tz = pytz.timezone(employee.tz or 'UTC')
            now_tz = now_utc.astimezone(tz)
            start_tz = now_tz.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            start_naive = start_tz.astimezone(pytz.utc).replace(tzinfo=None)
            end_tz = now_tz
            end_naive = end_tz.astimezone(pytz.utc).replace(tzinfo=None)

            hours = sum(
                att.worked_hours or 0
                for att in employee.attendance_ids.filtered(
                    lambda a: a.check_in >= start_naive and a.check_out and a.check_out <= end_naive
                )
            )
            employee.hours_last_month = round(hours, 2)
            employee.hours_last_month_display = "%g" % employee.hours_last_month

    def _compute_hours_today(self):
        now = fields.Datetime.now()
        now_utc = pytz.utc.localize(now)
        for employee in self:
            tz = pytz.timezone(employee.tz or 'UTC')
            now_tz = now_utc.astimezone(tz)
            start_tz = now_tz + relativedelta(hour=0, minute=0)
            start_naive = start_tz.astimezone(pytz.utc).replace(tzinfo=None)

            attendances = self.env['hr.attendance'].search([
                ('employee_id', 'in', employee.ids),
                ('check_in', '<=', now),
                '|', ('check_out', '>=', start_naive), ('check_out', '=', False),
            ], order='check_in asc')

            hours_previously_today = 0.0
            worked_hours = 0.0
            attendance_worked_hours = 0.0
            for attendance in attendances:
                delta = (attendance.check_out or now) - max(attendance.check_in, start_naive)
                attendance_worked_hours = delta.total_seconds() / 3600.0
                worked_hours += attendance_worked_hours
                hours_previously_today += attendance_worked_hours

            employee.last_attendance_worked_hours = attendance_worked_hours
            hours_previously_today -= attendance_worked_hours
            employee.hours_previously_today = hours_previously_today
            employee.hours_today = worked_hours

    @api.depends('attendance_ids')
    def _compute_last_attendance_id(self):
        for employee in self:
            employee.last_attendance_id = self.env['hr.attendance'].search([
                ('employee_id', 'in', employee.ids),
            ], order="check_in desc", limit=1)

    @api.depends('last_attendance_id.check_in', 'last_attendance_id.check_out', 'last_attendance_id')
    def _compute_attendance_state(self):
        for employee in self:
            att = employee.last_attendance_id.sudo()
            employee.attendance_state = 'checked_in' if (att and not att.check_out) else 'checked_out'

    # ------------------------------------------------------------
    # CORE ACTION (Check in / Check out)
    # ------------------------------------------------------------
    # def _attendance_action_change(self, geo_information=None):
    #     """Check In / Check Out action."""
    #     self.ensure_one()
    #     action_date = fields.Datetime.now()

    #     if self.attendance_state != 'checked_in':
    #         # CHECK IN
    #         vals = {'employee_id': self.id, 'check_in': action_date}
    #         if geo_information:
    #             vals.update({'in_%s' % k: v for k, v in geo_information.items()})
    #         return self.env['hr.attendance'].create(vals)

    #     # CHECK OUT
    #     attendance = self.env['hr.attendance'].search([
    #         ('employee_id', '=', self.id), ('check_out', '=', False)
    #     ], limit=1)
    #     if attendance:
    #         write_vals = {'check_out': action_date}
    #         if geo_information:
    #             write_vals.update({'out_%s' % k: v for k, v in geo_information.items()})
    #         attendance.write(write_vals)
    #     else:
    #         raise exceptions.UserError(_(
    #             'Cannot perform check out on %(empl_name)s, could not find corresponding check in. '
    #             'Your attendances have probably been modified manually by human resources.',
    #             empl_name=self.sudo().name
    #         ))
    #     return attendance
  
    def _attendance_action_change(self, geo_information=None):
        """Check In / Check Out with:
        - Check-in: create attendance + log note tagging Attendance Officers
        - Check-out: block if there is an open break
        """
        self.ensure_one()
        action_date = fields.Datetime.now()

        # ---------------------------
        # CHECK IN
        # ---------------------------
        if self.attendance_state != 'checked_in':
            vals = {'employee_id': self.id, 'check_in': action_date}
            if geo_information:
                vals.update({'in_%s' % k: v for k, v in geo_information.items()})
            attendance = self.env['hr.attendance'].create(vals)

            # Build log-note body in user's timezone
            local_dt = fields.Datetime.context_timestamp(
                self.env.user, action_date
            ).strftime("%Y-%m-%d %H:%M")
            body = _("%s checked in at %s.") % (self.name, local_dt)

            # Tag Attendance Officers (admins of Attendance)
            managers_grp = self.env.ref(
                'hr_attendance.group_hr_attendance_officer', raise_if_not_found=False
            )
            partners = managers_grp and managers_grp.users.mapped('partner_id') or self.env['res.partner']

            # Ensure they follow the record (optional but useful)
            if partners:
                attendance.message_subscribe(partner_ids=partners.ids)

            # Post as LOG NOTE (no email icon), tagging partners
            # Post as a pure LOG NOTE (no email). This triggers in-app notifications only.
            ctx = {
                "mail_notify_noemail": True,      # <<< force NO email creation for notifications
                "mail_notify_force_send": False,  # don't push anything to mail queue
                "mail_create_nosubscribe": True,  # don't auto-subscribe anyone
                "mail_post_autofollow": False,    # no auto-follow logic
                "tracking_disable": True,         # don't create tracking emails
            }
            attendance.with_context(**ctx).message_post(
                body=body,
                message_type="comment",           # chatter comment (log note)
                subtype_xmlid="mail.mt_note",     # "Log note" subtype (never emails)
                partner_ids=partners.ids,         # tag Attendance Officers (in-app ping only)
                email_layout_xmlid=False,
                attachment_ids=[],
            )

            return attendance

        # ---------------------------
        # CHECK OUT
        # ---------------------------
        attendance = self.env['hr.attendance'].search([
            ('employee_id', '=', self.id),
            ('check_out', '=', False),
        ], limit=1)

        if not attendance:
            raise exceptions.UserError(_(
                'Cannot perform check out on %(empl_name)s, could not find corresponding check in. '
                'Your attendances have probably been modified manually by human resources.',
                empl_name=self.sudo().name,
            ))

        # Block check-out if a break is still open
        open_break = attendance.sudo().break_ids.filtered(lambda b: not b.check_out)[:1]
        if open_break:
            raise exceptions.UserError(_("Please Break Out before you Check Out."))

        # Proceed with check-out
        write_vals = {'check_out': action_date}
        if geo_information:
            write_vals.update({'out_%s' % k: v for k, v in geo_information.items()})
        attendance.write(write_vals)
        return attendance


    # ------------------------------------------------------------
    # UI ACTIONS
    # ------------------------------------------------------------
    def action_open_last_month_attendances(self):
        self.ensure_one()
        return {
            "type": "ir.actions.act_window",
            "name": _("Attendances This Month"),
            "res_model": "hr.attendance",
            "views": [[self.env.ref('hr_attendance.hr_attendance_employee_simple_tree_view').id, "list"]],
            "context": {"create": 0},
            "domain": [
                ('employee_id', '=', self.id),
                ('check_in', ">=", fields.datetime.today().replace(day=1, hour=0, minute=0)),
            ],
        }

    def action_open_last_month_overtime(self):
        self.ensure_one()
        return {
            "type": "ir.actions.act_window",
            "name": _("Attendances This Month"),
            "res_model": "hr.attendance",
            "views": [[self.env.ref('hr_attendance.hr_attendance_validated_hours_employee_simple_tree_view').id, "list"]],
            "context": {"create": 0},
            "domain": [('employee_id', '=', self.id), ('overtime_status', '=', 'approved')],
        }


    def _get_open_attendance(self):
        self.ensure_one()
        return self.env['hr.attendance'].sudo().search([
            ('employee_id', '=', self.id),
            ('check_out', '=', False),
        ], limit=1)

    def _get_break_status(self, attendance):
        """Return (has_any_break, has_open_break) for this attendance."""
        self.ensure_one()
        Break = self.env['hr.attendance.break'].sudo()
        if not attendance:
            return (False, False)
        has_any = bool(Break.search([('attendance_id', '=', attendance.id)], limit=1))
        has_open = bool(Break.search([
            ('attendance_id', '=', attendance.id),
            ('check_out', '=', False)
        ], limit=1))
        return (has_any, has_open)

   # BREAK ACTIONS
    def action_break_in(self, latitude=False, longitude=False):
        self.ensure_one()
        if self.attendance_state != 'checked_in':
            raise UserError(_("You must Check In before starting a break."))

        # Current open attendance
        open_att = self.env['hr.attendance'].search([
            ('employee_id', '=', self.id),
            ('check_out', '=', False)
        ], limit=1)
        if not open_att:
            raise UserError(_("No open attendance found. Please Check In first."))

        Break = self.env['hr.attendance.break'].sudo()

        # 🔒 Allow at most ONE break per attendance (your business rule)
        existing_for_att = Break.search([
            ('attendance_id', '=', open_att.id)
        ], limit=1)
        if existing_for_att:
            raise UserError(_("You already recorded a break for this attendance."))

        # ❗ Also guard against any orphan open break (stale)
        open_any = Break.search([
            ('employee_id', '=', self.id),
            ('check_out', '=', False)
        ], limit=1)
        if open_any:
            raise UserError(_("You already have an open break. Please Break Out first."))

        # Create break with check_in now
        vals = {
            'employee_id': self.id,
            'attendance_id': open_att.id,
            'check_in': fields.Datetime.now(),
        }
        if latitude and longitude:
            # only if aapke break model me ye fields exist karte hain
            vals.update({'in_latitude': latitude, 'in_longitude': longitude})

        Break.create(vals)
        return True


    def action_break_out(self, latitude=False, longitude=False):
        self.ensure_one()

        open_att = self.env['hr.attendance'].search([
            ('employee_id', '=', self.id),
            ('check_out', '=', False)
        ], limit=1)
        if not open_att:
            raise UserError(_("No open attendance found."))

        Break = self.env['hr.attendance.break'].sudo()
        # Close the break for this attendance (not “any”)
        open_break = Break.search([
            ('attendance_id', '=', open_att.id),
            ('check_out', '=', False)
        ], limit=1)

        if not open_break:
            raise UserError(_("No open break to close."))

        vals = {'check_out': fields.Datetime.now()}
        if latitude and longitude:
            # only if aapke break model me ye fields exist karte hain
            vals.update({'out_latitude': latitude, 'out_longitude': longitude})
        open_break.write(vals)
        return True



    # Auto-close any open break when employee checks out manually
    def attendance_manual(self, next_action):
        res = super().attendance_manual(next_action)
        for emp in self:
            if emp.attendance_state == 'checked_out':
                brk = emp._get_open_break()
                if brk:
                    brk.sudo().write({
                        'check_out': fields.Datetime.now(),
                        'state': 'done',
                    })
        return res