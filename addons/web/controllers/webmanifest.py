...
class WebManifest(http.Controller):
    ...
    def _get_webmanifest(self):
        web_app_name = request.env['ir.config_parameter'].sudo().get_param('web.web_app_name', 'AeboCRM')
        manifest = {
            'name': web_app_name,
            'scope': '/odoo',
            'start_url': '/odoo',
        ...
        manifest['icons'] = [{
            'src': '/web/static/img/aebo-icon-%s.png' % size,
            'sizes': size,
            'type': 'image/png',
    ...
    def _icon_path(self):
        return 'web/static/img/aebo-icon-192x192.png'
    ...
    def scoped_app(self, app_id, path='', app_name=''):
        ...
        scoped_app_values = {
            'app_id': app_id,
            'apple_touch_icon': '/web/static/img/aebo-icon-ios.png',
            'app_name': app_name,
            'path': path,
