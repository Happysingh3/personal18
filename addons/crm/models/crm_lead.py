...
class Lead(models.Model):
    ...
    @api.depends('user_id', 'type')  # same deps so compute triggers, but we keep team if set
    def _compute_team_id(self):
        """
        Preserve existing team. Only compute a team if none is set.
        This prevents user change from moving a Tender lead to Sales team.
        """
        for lead in self:
            # if a team is already chosen, DO NOT change it
            if lead.team_id:
                # optional: keep company consistent with team
                if lead.team_id.company_id and lead.company_id != lead.team_id.company_id:
                    lead.company_id = lead.team_id.company_id
                continue

            # otherwise, fallback to Odoo's default logic to pick a team
            if not lead.user_id:
                lead.team_id = False
                continue

            team_domain = [('use_leads', '=', True)] if lead.type == 'lead' else [('use_opportunities', '=', True)]
            default_team = self.env['crm.team']._get_default_team_id(user_id=lead.user_id.id, domain=team_domain)
            lead.team_id = default_team.id if default_team else False
