...
class MasterSearch(models.Model):
    ...
    crm_count = fields.Integer(string="CRM Count",
                           compute="_get_crm_count",
                           help='To fetch CRM search count')
    ...
    'master_search_crm_lead_rel',
    'search_id', 'lead_id',
    string="CRM Leads",
    help='To fetch CRM Leads matching search'
)
    ...
    def _get_crm_count(self):
        """Get CRM lead count"""
        self.crm_count = len(self.crm_lead_ids)
    ...
    def _search_query(self, key):
        ...
        self._search_crm_leads(key, active_qry, company_id) \
        if self.search_by in ['any', 'crm details'] else False
    ...
    def _search_customer(self, key, active_qry):
        ...
        (r.name ILIKE '%{key}%' OR   r.street ILIKE '%{key}%' OR 
         ...
         r.zip ILIKE '%{key}%' OR  r.email ILIKE '%{key}%' OR r.phone ILIKE '%{key}%')   """
